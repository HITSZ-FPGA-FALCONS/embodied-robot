...
module tb_pwm_gen;
    ...
    initial begin
        ...
        // 等待 pwm 变高。一直等不到会被全局超时抓到。
        //
        // ⚠️ 这里有个"差一拍"的坑，值得写清楚：
        //     紧跟在 @(posedge clk) 后面读 pwm，读到的是**【这一拍更新前】**的值；
        //     而中间加一个 #1 之后再读，读到的是**更新后**的值。两者差一拍。
        //
        //     所以不能写成 `while (!pwm) @(posedge clk);` 然后再 `#1; check(...)`——
        //     循环退出时看到的是高，检查时看的却是下一拍，在 duty 较小或
        //     恰好退在高电平最后一拍时，**正确的设计也会被误报失败**。
        //
